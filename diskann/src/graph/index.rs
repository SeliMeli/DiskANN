        ...
        async move {
            ...
            for attempt in 0..num_insert_attempts {
                ...
                let context = prune::Context {
                    ...
                    available: &mut prune_scratch.available,
    ...
    DP: DataProvider,
{
        ...
        Itr: ExactSizeIterator<Item = DP::InternalId> + Clone + Send + Sync,
    {
        async move {
            ...
            let mut context = prune::Context {
                ...
                available: &mut scratch.available,
        ...
        F: Fn(DP::InternalId) -> bool,
    {
        ...
        prune::robust_prune_parts(
            ...
            policy,
            |candidate, selected| {
                let Some(candidate) = map.get(candidate) else {
                    return Ok(f32::MAX);
                };
                let Some(selected) = map.get(selected) else {
                    return Ok(f32::MAX);
                };
                Ok(computer.evaluate_similarity(candidate.reborrow(), selected.reborrow()))
            },
            |id| map.get(id).is_some(),
            exclude,
        )
    }
