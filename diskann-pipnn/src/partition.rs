...
//! Distance and top-k selection for partition assignment.
//!
    ...
    F: SIMDVector<Scalar = f32> + SIMDPartialOrd,
{
    ...
    for (lane, &value) in values[..F::LANES].iter().enumerate() {
        if eligible.get_unchecked(lane) {
            insert_topk(top, fanout, (base + lane) as u32, value);
        }
    }
}
...
mod tests {
    ...
    fn assert_architectures(input: PartitionTopK<'_>, fanout: usize) {
        let expected = expected(input, fanout);

        let mut scalar = vec![u32::MAX; expected.len()];
        diskann_wide::arch::Scalar::new().run(PartitionKernel {
            input,
            fanout,
            output: &mut scalar,
        });
        ...
        if let Some(arch) = diskann_wide::arch::x86_64::V3::new_checked() {
            ...
            arch.run(PartitionKernel {
                input,
                fanout,
                output: &mut output,
            });
            assert_eq!(output, expected, "V3");
        }
        ...
        if let Some(arch) = diskann_wide::arch::aarch64::Neon::new_checked() {
            ...
            assert_eq!(output, expected, "Neon");
        }
    }

    fn input_for(metric: Metric, leaders: usize) -> (Vec<f32>, Vec<f32>, Vec<f32>) {
    ...
    fn every_available_architecture_matches_scalar_across_width_boundaries() {
        for metric in [
            ...
            for leaders in [7, 8, 9, 15, 16, 17] {
                let (dots, row_scales, leader_scales) = input_for(metric, leaders);
