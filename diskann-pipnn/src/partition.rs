...
pub fn partition<T: VectorRepr + Send + Sync>(
    ...
    // Global merge: collect all under-c_min leaves and merge them.
    let c_min = config.c_min;
    let c_max = config.c_max;
    let (mut good, small): (Vec<Leaf>, Vec<Leaf>) =
        leaves.into_iter().partition(|l| l.indices.len() >= c_min);

    if !small.is_empty() {
        // Merge small leaves together using the same buf approach.
        let mut buf = std::collections::HashSet::<u32>::new();
        for leaf in small {
            if buf.len() + leaf.indices.len() > c_max {
                good.push(Leaf {
        ...
        // Final remainder — merge into last leaf if possible.
        if !buf.is_empty() {
...
fn partition_one_level<T: VectorRepr + Send + Sync>(
    ...
    let num_leaders = {
        let _timer = crate::profile::PhaseTimer::start("partition/leader_sample");
        sample_num_leaders(n, config.p_samp, config.leader_cap)
    };
    ...
    // No per-level merge_small — all small clusters go to the global merge at the end.
    let clusters = {
        let _timer = crate::profile::PhaseTimer::start("partition/assign_to_leaders");
        assign_to_leaders(data, ndims, &item.indices, &leaders, fanout, config.metric)
    };

    let mut next_work = Vec::new();
    let mut finished_leaves = Vec::new();
    {
        let _timer = crate::profile::PhaseTimer::start("partition/merge_small");
...
/// Release thread-local stripe buffers to free memory after partition is done.
pub fn release_stripe_buffers() {
...
fn assign_to_leaders<T: VectorRepr + Send + Sync>(
    ...
    {
        let _timer = crate::profile::PhaseTimer::start("partition/assign/leader_conv");
    ...
    let l_norms: Vec<f32> = {
        let _timer = crate::profile::PhaseTimer::start("partition/assign/leader_norms");
        match metric {
            Metric::L2 => l_data
                .chunks_exact(ndims)
    ...
    let process_stripe = |(stripe_idx, assign_chunk): (usize, &mut [u32])| {
        STRIPE_BUFS.with(|cell| {
            ...
            {
                let _timer = crate::profile::PhaseTimer::start("partition/assign/gemm");
            ...
            {
                let _timer = crate::profile::PhaseTimer::start("partition/assign/topk");
                for i in 0..sn {
    ...
    {
        let _timer = crate::profile::PhaseTimer::start("partition/assign/aggregate");
...
fn merge_small(clusters: Vec<Vec<u32>>, c_min: usize, c_max: usize) -> Vec<Vec<u32>> {
    ...
    for c in clusters {
        ...
        if c.len() >= c_min {
            ...
            buf.extend(c);
            // Flush once buf reaches c_min — avoids accumulating then leaving
            // a small remainder at the end.
            if buf.len() >= c_min {
                result.push(buf.drain().collect());
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_assign_to_leaders_cosine_normalized_fanout() {
        let data = vec![1.0f32, 0.0, 0.0, 1.0, 0.8, 0.6, -1.0, 0.0, 0.0, -1.0];
        let points = vec![0, 1, 2, 3, 4];
        let leaders = vec![0, 1, 3];
        ...
        assert!(clusters[1].contains(&1));
        assert!(clusters[1].contains(&2));
        assert!(clusters[2].contains(&3));
        assert!(clusters[2].contains(&4));

        let assignment_count: usize = clusters.iter().map(Vec::len).sum();
        assert_eq!(assignment_count, points.len() * 2);
    }
}
